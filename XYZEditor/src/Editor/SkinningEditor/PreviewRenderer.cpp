	...
	namespace Editor {
        ...
        {         
            if (preview)
            {
                ...
                //    Renderer2D::SubmitEditorCircle(glm::vec3(vertex.Position.x, vertex.Position.y, 0.0f), SkinnedMesh::PointRadius, 20, mesh.m_Colors[SkinnedMesh::VertexColor]);
            }
            else
            {
                uint32_t counter = 0;
                ...
                for (auto& subMesh : mesh.m_Submeshes)
                {
                    ...
                    //    Renderer2D::SubmitEditorCircle(glm::vec3(vertex.Position.x, vertex.Position.y, 0.0f), SkinnedMesh::PointRadius, 20, subMesh.Color);
                }
            }
        }
        ...
        {
            //Renderer2D::SubmitEditorCircle(glm::vec3(start, 0.0f), radius, 20, color);
            //Renderer2D::SubmitEditorCircle(glm::vec3(end, 0.0f), radius, 20, color);
            //Renderer2D::SubmitEditorLine(glm::vec3(start + (normal * radius),0.0f), glm::vec3(end, 0.0f), color);
            //Renderer2D::SubmitEditorLine(glm::vec3(start - (normal * radius),0.0f), glm::vec3(end, 0.0f), color);
        }
        ...
        {
            //Renderer2D::SubmitEditorLine(glm::vec3(firstPosition.x, firstPosition.y, 0.0f), glm::vec3(secondPosition.x, secondPosition.y, 0.0f), color);
            //Renderer2D::SubmitEditorLine(glm::vec3(secondPosition.x, secondPosition.y, 0.0f), glm::vec3(thirdPosition.x, thirdPosition.y, 0.0f), color);
            //Renderer2D::SubmitEditorLine(glm::vec3(thirdPosition.x, thirdPosition.y, 0.0f), glm::vec3(firstPosition.x, firstPosition.y, 0.0f), color);
        }
