...
namespace XYZ {
	...
	VoxelChunk VoxelWorld::generateChunk(int64_t chunkX, int64_t chunkZ, const VoxelBiom& biom) const
	{
		VoxelChunk chunk;
		...
		for (uint32_t x = 0; x < submesh.Width; ++x)
		{
			for (uint32_t z = 0; z < submesh.Depth; ++z)
			{
				...
				const double val = Perlin::Octave2D(xDouble * fx, zDouble * fy, biom.Octaves);
