...
namespace XYZ {
	...
	void ScriptEngine::ReloadAssembly(const std::string& path)
	{
		...
		if (s_EntityInstanceMap.size())
		{
			...
			auto& entityMap = s_EntityInstanceMap.at(scene->Getuint32_t());
			for (auto&[entityID, entityInstanceData]: entityMap)
			{
				const auto& entityMap = scene->GetEntityMap();
				XYZ_ASSERT(entityMap.find(entityID) != entityMap.end(), "Invalid entity ID or entity doesn't exist in scene!");
	...
	void ScriptEngine::OnSceneDestruct(uint32_t sceneID)
	{
	...
	void ScriptEngine::CopyEntityScriptData(uint32_t dst, uint32_t src)
	{
		XYZ_ASSERT(s_EntityInstanceMap.find(dst) != s_EntityInstanceMap.end());
		XYZ_ASSERT(s_EntityInstanceMap.find(src) != s_EntityInstanceMap.end());
		...
		for (auto& [entityID, entityInstanceData] : srcEntityMap)
		{
			for (auto& [moduleName, srcFieldMap] : srcEntityMap[entityID].ModuleFieldMap)
			{
				...
				{
					XYZ_ASSERT(dstModuleFieldMap.find(moduleName) != dstModuleFieldMap.end());
					auto& fieldMap = dstModuleFieldMap.at(moduleName);
					XYZ_ASSERT(fieldMap.find(fieldName) != fieldMap.end());
	...
	{
		OnCreateEntity(entity.m_Scene->Getuint32_t(), entity.GetComponent<IDComponent>().ID);
	}

	void ScriptEngine::OnCreateEntity(uint32_t sceneID, uint32_t entityID)
	{
	...
	void ScriptEngine::OnUpdateEntity(uint32_t sceneID, uint32_t entityID, Timestep ts)
	{
		EntityInstance& entityInstance = GetEntityInstanceData(sceneID, entityID).Instance;
	...
	void ScriptEngine::OnScriptComponentDestroyed(uint32_t sceneID, uint32_t entityID)
	{
		XYZ_ASSERT(s_EntityInstanceMap.find(sceneID) != s_EntityInstanceMap.end());
		auto& entityMap = s_EntityInstanceMap.at(sceneID);
		XYZ_ASSERT(entityMap.find(entityID) != entityMap.end());
	...
	void ScriptEngine::InitScriptEntity(Entity entity)
	{
		...
		uint32_t id = entity.GetComponent<IDComponent>().ID;
		...
		{
			XYZ_ERROR("Entity references non-existent script module '{0}'", moduleName);
			return;
		}
		...
		EntityInstanceData& entityInstanceData = s_EntityInstanceMap[scene->Getuint32_t()][id];
	...
	{
		EntityInstanceData& entityInstanceData = GetEntityInstanceData(entity.GetSceneuint32_t(), entity.Getuint32_t());
		ScriptModuleFieldMap& moduleFieldMap = entityInstanceData.ModuleFieldMap;
	...
	void ScriptEngine::InstantiateEntityClass(Entity entity)
	{
		...
		EntityInstanceData& entityInstanceData = GetEntityInstanceData(scene->Getuint32_t(), id);
		EntityInstance& entityInstance = entityInstanceData.Instance;
		XYZ_ASSERT(entityInstance.ScriptClass);
	...
	EntityInstanceData& ScriptEngine::GetEntityInstanceData(uint32_t sceneID, uint32_t entityID)
	{
		XYZ_ASSERT(s_EntityInstanceMap.find(sceneID) != s_EntityInstanceMap.end(), "Invalid scene ID!");
		auto& entityIDMap = s_EntityInstanceMap.at(sceneID);
		XYZ_ASSERT(entityIDMap.find(entityID) != entityIDMap.end(), "Invalid entity ID!");
	...
	{
		XYZ_ASSERT(m_EntityInstance->GetInstance());
