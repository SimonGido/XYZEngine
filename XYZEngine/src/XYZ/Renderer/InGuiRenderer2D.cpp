...
namespace XYZ {
	...
	struct InGuiRenderer2DData
	{
		...
		InGuiVertex2D* BufferBase = nullptr;
		InGuiVertex2D* BufferPtr = nullptr;
		...
		InGuiRenderer2DStats Stats;
	};


	void InGuiRenderer2DData::Reset()
	{
		if (!BufferBase)
		{
			DefaultQuadMaterial = Ref<Material>::Create(Shader::Create("Assets/Shaders/InGuiShader.glsl"));
			BufferBase = new InGuiVertex2D[MaxVertices];
			QuadVertexArray = VertexArray::Create();
			QuadVertexBuffer = VertexBuffer::Create(MaxVertices * sizeof(InGuiVertex2D));
			...
			{4, XYZ::ShaderDataComponent::Float,  "a_ScissorIndex" }
				});
	...
	void InGuiRenderer2DData::ResetLines()
	{
		if (!LineBufferBase)
		{	// Lines
	...
	static InGuiRenderer2DData s_Data;

	void InGuiRenderer2D::Init()
	{
		s_Data.Reset();
		s_Data.ResetLines();
	}

	void InGuiRenderer2D::Shutdown()
	{
		delete[] s_Data.BufferBase;
		delete[] s_Data.LineBufferBase;
	}

	void InGuiRenderer2D::BeginScene(const glm::mat4& viewProjectionMatrix)
	{
		s_Data.ViewProjectionMatrix = viewProjectionMatrix;
		s_Data.QuadMaterial = s_Data.DefaultQuadMaterial;
	}

	uint32_t InGuiRenderer2D::SetTexture(const Ref<Texture>& texture)
	{
	...
	void InGuiRenderer2D::SetMaterial(const Ref<Material>& material)
	{
	...
	void InGuiRenderer2D::SubmitLine(const glm::vec3& p0, const glm::vec3& p1, const glm::vec4& color)
	{
	...
	void InGuiRenderer2D::SubmitQuadNotCentered(const glm::vec3& position, const glm::vec2& size, const glm::vec4& texCoord, uint32_t textureID, const glm::vec4& color, uint32_t scissorIndex)
	{
		...
		for (size_t i = 0; i < quadVertexCount; ++i)
		{
			...
			s_Data.BufferPtr->ScissorIndex = (float)scissorIndex;
			s_Data.BufferPtr++;
		}
		s_Data.IndexCount += 6;
	}
	...
	void InGuiRenderer2D::FlushLines()
	{
