...
namespace XYZ {
	...
	{
		XYZ_ASSERT(scale % 2 == 0, "Scale must be divisible by two");
					...
					else
					{
						...
						while (subScale >= 1)
						{
							...
							if (allSubUniform)
							{
								...
								compressedColorIndices.resize(offset + cell.VoxelCount, 0);
	...
	bool VoxelSubmesh::Decompress()
	{
		if (!Compressed)
			return 0;

		VoxelSubmesh decompressed;
		decompressed.Width = Width * CompressScale;
		decompressed.Height = Height * CompressScale;
		decompressed.Depth = Depth * CompressScale;
		decompressed.VoxelSize = VoxelSize / CompressScale;
		decompressed.ColorIndices.resize(static_cast<size_t>(decompressed.Width) * decompressed.Height * decompressed.Depth);

		uint32_t colorDataSize = static_cast<uint32_t>(ColorIndices.size());
		uint32_t scale = CompressScale;

		// Iterate through all compressed cells
		for (uint32_t cx = 0; cx < Width; ++cx)
		{
			for (uint32_t cy = 0; cy < Height; ++cy)
			{
				...
				{
					const uint32_t cIndex = Index3D(cx, cy, cz, Width, Height);
					const CompressedCell& cell = CompressedCells[cIndex];

					const uint32_t xStart = cx * scale;
					const uint32_t yStart = cy * scale;
					const uint32_t zStart = cz * scale;
					const uint32_t xEnd = std::min(xStart + scale, decompressed.Width);
					const uint32_t yEnd = std::min(yStart + scale, decompressed.Height);
					const uint32_t zEnd = std::min(zStart + scale, decompressed.Depth);

					if (cell.VoxelCount == 1)
					{
						// Single color fills the entire block
						if (cell.VoxelOffset >= colorDataSize)
							continue; // safety guard

						uint8_t colorIndex = ColorIndices[cell.VoxelOffset];
						for (uint32_t x = xStart; x < xEnd; ++x)
						{
							for (uint32_t y = yStart; y < yEnd; ++y)
							{
								...
								{
									decompressed.ColorIndices[Index3D(x, y, z, decompressed.Width, decompressed.Height)] = colorIndex;
								}
							}
						}
					}
					...
					{
						// Subdivided cell (multiple colors stored)
						uint32_t blockCount = cell.VoxelCount;
						uint32_t subDiv = static_cast<uint32_t>(std::cbrt(blockCount));
						if (subDiv == 0) subDiv = 1;

						uint32_t subScale = scale / subDiv;
						uint32_t offset = cell.VoxelOffset;

						for (uint32_t sx = 0; sx < subDiv; ++sx)
						{
							for (uint32_t sy = 0; sy < subDiv; ++sy)
							{
								for (uint32_t sz = 0; sz < subDiv; ++sz)
								{
									uint32_t localIndex = Index3D(sx, sy, sz, subDiv, subDiv);
									if (offset + localIndex >= colorDataSize)
										continue;

									uint8_t colorIndex = ColorIndices[offset + localIndex];

									// Fill that sub-block with the color
									for (uint32_t x = xStart + sx * subScale; x < std::min(xStart + (sx + 1) * subScale, decompressed.Width); ++x)
									{
										for (uint32_t y = yStart + sy * subScale; y < std::min(yStart + (sy + 1) * subScale, decompressed.Height); ++y)
										{
											for (uint32_t z = zStart + sz * subScale; z < std::min(zStart + (sz + 1) * subScale, decompressed.Depth); ++z)
											{
												decompressed.ColorIndices[Index3D(x, y, z, decompressed.Width, decompressed.Height)] = colorIndex;
											}
										}
		...
		int64_t restoredVoxels = static_cast<int64_t>(decompressed.ColorIndices.size());
		*this = std::move(decompressed);
		Compressed = false;
		return restoredVoxels;
	}
