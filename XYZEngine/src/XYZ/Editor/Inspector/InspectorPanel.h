...
#include "XYZ/Editor/Inspector/Components/ComponentInspectors.h"
...
namespace XYZ {
	namespace Editor {
		...
		private:
			std::unordered_map<std::string, Ref<InspectorEditable>> m_Editables;

			std::vector<WeakRef<InspectorEditable>> m_EditablesInUse;
		...
		inline void InspectorPanel::RegisterEditable(const std::string& name, Args && ...args)
		{
			...
			static_assert(std::is_base_of_v<InspectorEditable, T>, "Editable must inherit from InspectorEditable");
